...
describe('cli session-proof', () => {
  ...
  test('reports provider-bound channel-session evidence for discord targets', () => {
    const configDir = fs.mkdtempSync(path.join(os.tmpdir(), 'clawbridge-cli-session-proof-discord-'));
    const openclawRoot = path.join(configDir, '.openclaw');
    const openclawConfigPath = path.join(openclawRoot, 'openclaw.json');
