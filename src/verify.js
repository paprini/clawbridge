...
check('agent-to-agent dispatch readiness', () => {
  ...
  if (defaultDeliveryKind === 'direct' && defaultDeliveryChannel && defaultDeliveryTarget) {
    const sessionStore = loadOpenClawSessionRows(tokenPath, configuredDispatchAgentId || 'main');
    const matchingRows = findMatchingDirectSessionRows(sessionStore.sessions, {
      channel: defaultDeliveryChannel,
      target: defaultDeliveryTarget,
    });

    if (matchingRows.some((row) => isProviderBoundDirectSessionRow(row, defaultDeliveryChannel, defaultDeliveryTarget))) {
      console.log(`    ℹ️  Found provider-bound direct OpenClaw session for ${defaultDeliveryChannel}:${defaultDeliveryTarget}.`);
      return true;
    }

    if (matchingRows.length > 0) {
      const matchedKeys = matchingRows
        .map((row) => row?.key)
        .filter(Boolean)
        .join(', ');
      return `OpenClaw currently routes ${defaultDeliveryChannel} direct chat for "${defaultDeliveryTarget}" into ${matchedKeys} instead of a provider-bound direct session. Remote session-first @agent delivery will return binding_required until this provider/session behavior changes on the target node.`;
    }

    console.log(`    ℹ️  No provider-bound direct OpenClaw session exists yet for ${defaultDeliveryChannel}:${defaultDeliveryTarget}. Have that user message the agent once before testing @agent from another node.`);
  }
